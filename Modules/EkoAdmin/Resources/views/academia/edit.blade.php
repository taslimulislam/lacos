...
@section('content')
    ...
    <div class="body-content">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{route('admin.academia.update',$academia->uuid)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="cust_border form-group mb-3 mx-0 pb-3 row">
                                ...
                                <div class="col-lg-10 pe-0">
                                    <input type="text" class="form-control" name="name" placeholder="" value="{{$academia->name}}" required>
                                    @if ($errors->has('name'))
                                ...
                                <div class="col-lg-10 pe-0">
                                    <select type="text" class="form-select search_test" name="industry_id" required>
                                        ...
                                        @foreach($industries as $industry)
                                            <option value="{{$industry->id}}" <?php if($industry->id == $academia->industry_id){echo "selected";}?> >{{$industry->name}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-lg-10 pe-0">
                                    <select class="form-select search_test" id="" name="country_id" required>
                                        ...
                                        @foreach($countries as $country)
                                            <option value="{{$country->id}}" <?php if($country->id == $academia->country_id){echo "selected";}?> >{{$country->country_name}}</option>
                                        @endforeach
                                    </select>
