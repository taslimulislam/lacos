<div class="cust_border form-group mb-3 mx-0 pb-3 row">
    <label for="{{ __('language.'.$input_name)  }}" class="col-sm-2 col-form-label ps-0">
        @if(isset($label)) {{$label}} @else {{ __('language.'.$input_name)  }} @endif
    ...
    <div class="col-lg-10 pe-0">
        <select name="{{ $input_name }}"
            ...
            @foreach($data_set as $key => $_data)
                ...
                <option value="{{ isset($data_value) ? $data_value : $_data}}" @if(old($input_name)) {{old($input_name) == $_data ? 'selected' : ''}}
